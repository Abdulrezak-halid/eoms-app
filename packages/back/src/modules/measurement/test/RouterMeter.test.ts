...
describe("E2E - RouterMeter", () => {
  ...
  it("should share meter with a partner and list shared partners/meters", async () => {
    const { client } = await UtilTest.createClientLoggedIn();
    const partnerToken = await client.PUT("/u/organization/partner/token");
    ...
    expect(createRes).toBeApiOk();
    const meterId = createRes.data!.id;

    const shareRes = await client.PUT("/u/measurement/meter/share/{id}", {
      params: { path: { id: meterId } },
      body: { partnerIds: [org2.session.orgId] },
    });
    expect(shareRes).toBeApiOk();

    const sharedPartnersRes = await client.GET(
      "/u/measurement/meter/shared-partners/{id}",
      { params: { path: { id: meterId } } },
    );
    ...
    const sharedMetersRes = await client.GET(
      "/u/measurement/meter/shared-meters",
    );
    expect(sharedMetersRes).toBeApiOk();
    expect(sharedMetersRes.data).toStrictEqual({
      records: [
        {
          id: meterId,
          name: "Shared Meter",
  ...
  it("partner should be able to get shared meters via getAll with or without partnerId", async () => {
    const { client, session } = await UtilTest.createClientLoggedIn();
    const partnerToken = await client.PUT("/u/organization/partner/token");
    ...
    await client.PUT("/u/measurement/meter/share/{id}", {
      params: { path: { id: meterId } },
      body: { partnerIds: [org2.session.orgId] },
    });

    const dateQuery = TestHelperMetric.getFullDatetimeRangeQuery();

    const getAllRes = await org2.client.GET("/u/measurement/meter/item", {
      params: { query: dateQuery },
    });
    expect(getAllRes).toBeApiOk();

    const fetchedMeter = getAllRes.data!.records.find((r) => r.id === meterId);
    expect(fetchedMeter).toBeDefined();
    expect(fetchedMeter).toMatchObject({
      id: meterId,
      name: "Partnered Meter",
      energyResource: "GAS",
    });
    ...
    const getByPartnerIdRes = await org2.client.GET(
      ...
      {
        params: { query: { ...dateQuery, partnerId: session.orgId } },
      },
    );
    expect(getByPartnerIdRes).toBeApiOk();
    expect(getByPartnerIdRes.data!.records.length).toBe(1);
    expect(getByPartnerIdRes.data!.records[0].id).toBe(meterId);

    const getSelfPartnerIdRes = await client.GET("/u/measurement/meter/item", {
      params: { query: { ...dateQuery, partnerId: session.orgId } },
    });
    expect(getSelfPartnerIdRes).toBeApiOk();
    expect(
      getSelfPartnerIdRes.data!.records.some((r) => r.id === meterId),
    ).toBeTruthy();
  });

  it("partner should be able to remove (unshare) a meter from their own list", async () => {
    const { client, session } = await UtilTest.createClientLoggedIn();
    const partnerToken = await client.PUT("/u/organization/partner/token");
    ...
    let org2Meters = await org2.client.GET("/u/measurement/meter/item", {
      params: { query: { ...dateQuery, partnerId: session.orgId } },
    });
    expect(org2Meters.data!.records.length).toBe(1);

    const removeShareRes = await org2.client.DELETE(
      "/u/measurement/meter/shared/{id}",
      {
        params: { path: { id: meterId } },
      },
    );
    ...
    org2Meters = await org2.client.GET("/u/measurement/meter/item", {
      params: { query: { ...dateQuery, partnerId: session.orgId } },
    });
    expect(org2Meters).toBeApiOk();
    expect(org2Meters.data!.records.length).toBe(0);

    const org1Meters = await client.GET("/u/measurement/meter/item/{id}", {
      params: { path: { id: meterId }, query: dateQuery },
    });
    expect(org1Meters).toBeApiOk();
  });
});
