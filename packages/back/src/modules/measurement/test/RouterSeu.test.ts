...
describe("E2E - RouterSeu", () => {
  ...
  it("should share SEU with a partner and list shared partners/seus", async () => {
    ...
    const createRes = await org1.client.POST("/u/measurement/seu/item", {
      body: {
        name: "Shared SEU",
        departmentIds: [],
        meterSliceIds: [],
    ...
    expect(createRes).toBeApiOk();
    const seuId = createRes.data!.id;

    const shareRes = await org1.client.PUT("/u/measurement/seu/share/{id}", {
      params: { path: { id: seuId } },
      body: { partnerIds: [org2.session.orgId] },
    });
    expect(shareRes).toBeApiOk();

    const sharedPartnersRes = await org1.client.GET(
      "/u/measurement/seu/shared-partners/{id}",
      { params: { path: { id: seuId } } },
    );
    ...
    const sharedSeusRes = await org1.client.GET(
      "/u/measurement/seu/shared-seus",
    );
    expect(sharedSeusRes).toBeApiOk();
    expect(sharedSeusRes.data).toStrictEqual({
      records: [
        {
          id: seuId,
          name: "Shared SEU",
  ...
  it("partner should be able to get shared seus via getAll with or without partnerId", async () => {
    const org1 = await UtilTest.createClientLoggedIn();
    const partnerToken = await org1.client.PUT("/u/organization/partner/token");
    ...
    await org1.client.PUT("/u/measurement/seu/share/{id}", {
      params: { path: { id: seuId } },
      body: { partnerIds: [org2.session.orgId] },
    });
    ...
    const getAllRes = await org2.client.GET("/u/measurement/seu/item", {
      params: { query: dateQuery },
    });
    expect(getAllRes).toBeApiOk();

    const fetchedSeu = getAllRes.data!.records.find((r) => r.id === seuId);
    expect(fetchedSeu).toBeDefined();
    expect(fetchedSeu).toMatchObject({
      id: seuId,
      name: "Partnered SEU",
      energyResource: "GAS",
    });

    const getByPartnerIdRes = await org2.client.GET("/u/measurement/seu/item", {
      params: { query: { ...dateQuery, partnerId: org1.session.orgId } },
    });
    expect(getByPartnerIdRes).toBeApiOk();
    expect(getByPartnerIdRes.data!.records.length).toBe(1);
    expect(getByPartnerIdRes.data!.records[0].id).toBe(seuId);
    ...
    expect(
      getSelfPartnerIdRes.data!.records.some((r) => r.id === seuId),
    ).toBeTruthy();
  });

  it("partner should be able to remove (unshare) a seu from their own list", async () => {
    const org1 = await UtilTest.createClientLoggedIn();
    const partnerToken = await org1.client.PUT("/u/organization/partner/token");
    ...
    let org2Seus = await org2.client.GET("/u/measurement/seu/item", {
      params: { query: { ...dateQuery, partnerId: org1.session.orgId } },
    });
    expect(org2Seus.data!.records.length).toBe(1);

    const removeShareRes = await org2.client.DELETE(
      "/u/measurement/seu/shared/{id}",
      {
        params: { path: { id: seuId } },
      },
    );
    ...
    org2Seus = await org2.client.GET("/u/measurement/seu/item", {
      params: { query: { ...dateQuery, partnerId: org1.session.orgId } },
    });
    expect(org2Seus).toBeApiOk();
    expect(org2Seus.data!.records.length).toBe(0);

    const org1Seus = await org1.client.GET("/u/measurement/seu/item/{id}", {
      params: { path: { id: seuId }, query: dateQuery },
    });
    expect(org1Seus).toBeApiOk();
  });
