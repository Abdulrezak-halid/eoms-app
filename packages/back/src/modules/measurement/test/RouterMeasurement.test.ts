import { EApiFailCode } from "common";
import { describe, expect, it } from "vitest";

import { UtilTest } from "@/test/utils/UtilTest";

import { TestHelperMetric } from "./TestHelperMetric";

describe("RouterMeasurement", () => {
  it("should be return without permission should response FORBIDDEN error", async () => {
    const { client } = await UtilTest.createClientLoggedIn({
      overwrittenPermissions: [],
    });
    const res = await client.GET("/u/measurement/seu/item", {
      params: {
        query: TestHelperMetric.getFullDatetimeRangeQuery(),
      },
    });
    expect(res).toBeApiError(EApiFailCode.FORBIDDEN);
  });

  it("should be returnwith a minimal permission should response ok", async () => {
    const { client } = await UtilTest.createClientLoggedIn({
      overwrittenPermissions: ["/MEASUREMENT"],
    });
    const res = await client.GET("/u/measurement/seu/item", {
      params: {
        query: TestHelperMetric.getFullDatetimeRangeQuery(),
      },
    });
    expect(res).toBeApiOk();
  });

  it("should respond PLAN_DISABLED_OP error without correct organization plan feature", async () => {
    const { client } = await UtilTest.createClientLoggedIn({
      overwrittenOrganizationPlan: { features: [] },
    });
    const res = await client.GET("/u/measurement/seu/item", {
      params: {
        query: TestHelperMetric.getFullDatetimeRangeQuery(),
      },
    });
    expect(res).toBeApiError(EApiFailCode.PLAN_DISABLED_OP);
  });

  it("should work with correct organization plan feature", async () => {
    const { client } = await UtilTest.createClientLoggedIn({
      overwrittenOrganizationPlan: { features: ["MEASUREMENT"] },
    });
    const res = await client.GET("/u/measurement/seu/item", {
      params: {
        query: TestHelperMetric.getFullDatetimeRangeQuery(),
      },
    });
    expect(res).toBeApiOk();
  });
});
