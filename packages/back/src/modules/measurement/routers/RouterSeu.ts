...
RouterSeu.openapi(
  ...
  async (c) => {
    const records = await ServiceSeu.getSharedSeus(c.var);
    return c.json({ records });
  },
);
...
RouterSeu.openapi(
  ...
  async (c) => {
    ...
    const records = await ServiceSeu.getSharedPartners(c.var, param.id);
    return c.json({ records });
  },
);
...
RouterSeu.openapi(
  ...
  async (c) => {
    ...
    await ServiceSeu.shareWithPartners(c.var, param.id, json.partnerIds);
    return UtilHono.resNull(c);
  },
);
...
RouterSeu.openapi(
  ...
  async (c) => {
    const param = c.req.valid("param");
    await ServiceSeu.removeSharedSeu(c.var, param.id);
    return UtilHono.resNull(c);
  },
);
