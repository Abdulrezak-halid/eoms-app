...
import {
  SchemaMetricResourceLabel,
  SchemaMetricResourceLabelWithoutType,
} from "../schemas/SchemaMetricResourceLabel";
...
RouterMetric.openapi(
  createRoute({
    method: "get",
    path: "/item",
    request: UtilOpenApi.genRequestQuery({ partnerId: SchemaUuid.optional() }),
    responses: UtilOpenApi.genResponseJson(
      z.object({
        records: z.array(
          z.object({
...
RouterMetric.openapi(
  ...
  async (c) => {
    const records = await ServiceMetric.getSharedMetrics(c.var);
    return c.json({ records });
  },
);
...
RouterMetric.openapi(
  ...
  async (c) => {
    const param = c.req.valid("param");
    const records = await ServiceMetric.getSharedPartners(c.var, param.id);
    return c.json({ records });
  },
);
...
RouterMetric.openapi(
  ...
  async (c) => {
    ...
    await ServiceMetric.shareWithPartners(c.var, param.id, json.partnerIds);
    return UtilHono.resNull(c);
  },
);
...
RouterMetric.openapi(
  ...
  async (c) => {
    const param = c.req.valid("param");
    await ServiceMetric.removeSharedMetric(c.var, param.id);
    return UtilHono.resNull(c);
  },
);
...
RouterMetric.openapi(
  ...
  async (c) => {
    const query = c.req.valid("query");
    const labels = await ServiceMetric.getLabels(c.var, query.metricId);
