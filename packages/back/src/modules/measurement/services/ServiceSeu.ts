...
import { TbSeuPartnerShared } from "../orm/TbSeuPartnerShared";
...
export namespace ServiceSeu {
  ...
  export async function getAll(
    ...
    if (query.partnerId) {
      if (query.partnerId === c.orgId) {
        filters.push(eq(TbSeu.orgId, c.orgId));
      } else {
        filters.push(eq(TbSeu.orgId, query.partnerId));
        filters.push(eq(TbSeuPartnerShared.partnerId, c.orgId));
      }
    } else {
      filters.push(
        or(
          eq(TbSeu.orgId, c.orgId),
          eq(TbSeuPartnerShared.partnerId, c.orgId),
        )!,
      );
    }
    ...
    const records = await c.db
      ...
      .from(TbSeu)
      .innerJoin(TbOrganization, eq(TbOrganization.id, TbSeu.orgId))

      .leftJoin(
        TbSeuPartnerShared,
        and(
          eq(TbSeuPartnerShared.subjectId, TbSeu.id),
          eq(TbSeuPartnerShared.partnerId, c.orgId),
        ),
      )
      ...
      .where(and(...filters))
      .groupBy(TbSeu.id, TbOrganization.id)
  ...
  export async function getNames(c: IContextOrg) {
    return await c.db
      ...
      .orderBy(TbSeu.energyResource, TbSeu.name);
  }
  ...
  export async function shareWithPartners(
    ...
    await c.db.transaction(async (tx) => {
      await tx
        .delete(TbSeuPartnerShared)
        .where(
          and(
            eq(TbSeuPartnerShared.orgId, c.orgId),
            eq(TbSeuPartnerShared.subjectId, id),
          ),
        );

      for (const pi of partnerIds) {
        await tx
          .insert(TbSeuPartnerShared)
  ...
  export async function getSharedPartners(c: IContextOrg, id: string) {
    return await c.db
      .select({
        id: TbSeuPartnerShared.partnerId,
        displayName: TbOrganization.displayName,
      })
      .from(TbSeuPartnerShared)
      .where(
        and(
          eq(TbSeuPartnerShared.subjectId, id),
          eq(TbSeuPartnerShared.orgId, c.orgId),
      ...
      .innerJoin(
        TbOrganization,
        eq(TbSeuPartnerShared.partnerId, TbOrganization.id),
      );
  }

  export async function getSharedSeus(c: IContextOrg) {
    return await c.db
      .select({
        id: TbSeuPartnerShared.subjectId,
        name: TbSeu.name,
      })
      .from(TbSeuPartnerShared)
      .where(eq(TbSeuPartnerShared.orgId, c.orgId))
      .innerJoin(TbSeu, eq(TbSeuPartnerShared.subjectId, TbSeu.id));
  }

  export async function removeSharedSeu(c: IContextOrg, id: string) {
    await c.db
      .delete(TbSeuPartnerShared)
      .where(
        and(
