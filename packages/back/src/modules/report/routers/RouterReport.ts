...
RouterReport.openapi(
  createRoute({
    ...
    path: "/send/{reportId}",
    request: {
      ...UtilOpenApi.genRequestParam({ reportId: SchemaUuid }),
      body: UtilOpenApi.genRequestJsonSub(
        z.object({
          email: SchemaEmail,
        }),
  ...
  async (c) => {
    ...
    const reportFile = await ServiceReport.getFile(c.var, param.reportId);
    const report = await ServiceReport.get(c.var, param.reportId);

    await ServiceMail.send(c.var, {
      to: json.email,
      subject: `Report: ${report.title.value}`,
      content: `<p>Hi!,</p><p>The titled report ${report.title.value} is attached.</p>`,
      attachments: [
        {
          filename: `${report.title.value}.pdf`,
          content: reportFile.buffer,
          contentType: reportFile.contentType || "application/pdf",
        },
      ],
    });
