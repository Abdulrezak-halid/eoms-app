...
import { SchemaPercentage } from "@m/core/schemas/SchemaPercentage";
...
RouterAdvancedRegression.openapi(
  createRoute({
    ...
    request: UtilOpenApi.genRequestJson(
      z.object({
        id: SchemaUuid,
        value: SchemaPercentage.min(1).nullable(),
      }),
    ),
    responses: UtilOpenApi.genResponseNull(),
  }),
  async (c) => {
    const json = c.req.valid("json");
    await ServiceAdvancedRegression.setThreshold(c.var, json.id, json.value);
    return UtilHono.resNull(c);
  },
);
