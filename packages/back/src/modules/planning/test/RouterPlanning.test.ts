...
describe("RouterPlanning", () => {
  it("GET /u/planning/target/item without permission should response FORBIDDEN error", async () => {
  ...
  it("GET /u/planning/target/item with a minimal permission should response ok", async () => {
    const { client } = await UtilTest.createClientLoggedIn({
      overwrittenPermissions: ["/PLANNING"],
    });
    const res = await client.GET("/u/planning/target/item");
