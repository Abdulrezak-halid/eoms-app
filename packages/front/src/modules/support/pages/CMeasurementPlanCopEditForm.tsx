...
export function CMeasurementPlanCopEditForm() {
  ...
  const fetcher = useCallback(async () => {
    const res = await Api.GET("/u/support/cop-measurement-plan/item/{id}", {
      params: { path: { id } },
    });
    return res;
  }, [id]);
  ...
  const handleSubmit = useCallback(
    async (body: IDtoMeasurementPlanCopRequest) => {
      const res = await Api.PUT("/u/support/cop-measurement-plan/item/{id}", {
        params: { path: { id } },
        body,
      });
      apiToast(res);
  ...
  const breadcrumbs = useMemo<IBreadCrumb[]>(
    () => [
      ...
      { label: data.payload?.energyVariables, dynamic: true },
      { label: t("edit") },
    ],
    [t, data.payload?.energyVariables],
  );
  return (
    <CBody breadcrumbs={breadcrumbs}>
      <CAsyncLoader data={data}>
        {(payload) => (
          <CMeasurementPlanCopForm
            initialData={payload}
