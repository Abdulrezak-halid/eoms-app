...
import { IDateQuickRange } from "@m/base/hooks/useQuickDateRanges";
...
export function CReportForm({
  ...
  const initialDateRange = useMemo<ICQuickDateRangeValue | undefined>(() => {
    if (!initialData?.content.dateStart) {
      return undefined;
    }

    if (!initialData.content.dateEnd) {
      return { quickRange: initialData.content.dateStart as IDateQuickRange };
    }

    return {
      customMin: initialData.content.dateStart,
      customMax: initialData.content.dateEnd,
    };
  }, [initialData?.content.dateStart, initialData?.content.dateEnd]);

  const inputDateRange = useInput<ICQuickDateRangeValue | undefined>(
    initialDateRange,
  );
  const dateRangeValidation = quickRangeToDateRange(inputDateRange.value, {
    required: !template,
  });
  ...
  const invalidMainForm = useInputInvalid(inputTitle, inputAuthorIds);
  ...
  const invalid = useMemo(() => {
    ...
    const invalidDateRange = Boolean(dateRangeValidation.invalidMsg);
    return (
      invalidMainForm ||
      invalidDateRange ||
      sections.length === 0 ||
    ...
    dateRangeValidation.invalidMsg,
    invalidAttachment,
    invalidMainForm,
    sections,
  ]);
  ...
  const handleSubmit = useCallback(async () => {
    if (invalid || dateRangeValidation.invalidMsg) {
      return;
    }
    ...
    await onSubmit({
      ...
      content: {
        ...
        dateStart:
          inputDateRange.value?.quickRange || inputDateRange.value?.customMin,
        dateEnd: inputDateRange.value?.quickRange
          ? undefined
          : inputDateRange.value?.customMax,
  ...
  return (
    <CForm onSubmit={handleSubmit}>
      <CFormPanel>
        ...
        <CFormLine
          label={t("dateRange")}
          invalidMsg={dateRangeValidation.invalidMsg}
        >
          <CQuickRangeSelect
            mode="dateOnly"
            value={inputDateRange.value}
            onChange={inputDateRange.onChange}
            invalid={Boolean(dateRangeValidation.invalidMsg)}
            required={!template}
          />
        </CFormLine>
        ...
        <div className="space-y-4">
          {sections.map((d, i) => (
            <CReportSection
              ...
              datetimeRange={inputDateRange.value}
