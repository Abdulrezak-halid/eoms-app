...
export const CReportSection = memo(function CReportSection({
  ...
  const inputCustomDateStart = useInput(data.customDate?.start);
  const inputCustomDateEnd = useInput(data.customDate?.end);
  const lastCustomDateRef = useRef<
    { start?: string; end?: string } | undefined
  >(data.customDate);

  const invalidForm = useInputInvalid(inputType, inputTitle);
  const invalidCustomDate = useInputInvalid(
    inputCustomDateStart,
    inputCustomDateEnd,
  );
  const invalid =
    invalidForm || invalidContent || (isCustomDateEnabled && invalidCustomDate);
  ...
  useEffect(() => {
    onDataChange(index, {
      ...
      customDate:
        isCustomDateEnabled &&
        inputCustomDateStart.value &&
        inputCustomDateEnd.value
          ? {
              start: inputCustomDateStart.value,
              end: inputCustomDateEnd.value,
            }
          : undefined,
  ...
  const handleEnableCustomDate = useCallback(() => {
    ...
    inputCustomDateStart.onChange(lastCustomDateRef.current?.start);
    inputCustomDateEnd.onChange(lastCustomDateRef.current?.end);
  }, [inputCustomDateEnd, inputCustomDateStart]);

  const handleDisableCustomDate = useCallback(() => {
    lastCustomDateRef.current = {
      start: inputCustomDateStart.value,
      end: inputCustomDateEnd.value,
    };
    setIsCustomDateEnabled(false);
  }, [inputCustomDateEnd.value, inputCustomDateStart.value]);
  ...
  const effectiveDatetimeRange = useMemo(() => {
    if (
      !isCustomDateEnabled ||
      !inputCustomDateStart.value ||
      !inputCustomDateEnd.value
    ...
    const endDate = UtilDate.localIsoDateToObj(inputCustomDateEnd.value);
      ...
      <CFormPanel>
        ...
        {isCustomDateEnabled && (
          <>
            <CFormLine
              label={t("startDate")}
              invalidMsg={inputCustomDateStart.invalidMsg}
            >
              <CInputDate
                {...inputCustomDateStart}
                required
                max={inputCustomDateEnd.value}
            ...
            <CFormLine
              label={t("endDate")}
              invalidMsg={inputCustomDateEnd.invalidMsg}
            >
              <CInputDate
                {...inputCustomDateEnd}
                required
                min={inputCustomDateStart.value}
