/**
 * @file: CDevNotificationBalloon.tsx
...
export function CDevNotificationBalloon() {
  const { push, remove } = useContext(ContextNotificationBalloon);

  const refIndex = useRef(1);

  const refControlledNotificationBalloonId = useRef<number | undefined>(
    undefined,
  );
  ...
  const handlePushControlled = useCallback(() => {
    if (refControlledNotificationBalloonId.current) {
      remove(refControlledNotificationBalloonId.current);
      refControlledNotificationBalloonId.current = undefined;
      return;
    }
    refControlledNotificationBalloonId.current = push(
      "Controlled.",
      "/",
      "danger",
      {
  ...
  return (
    <CBody title="Dev - Notification Balloons">
      <div className="space-y-4 flex flex-col">
        ...
        <div>Push Notification Balloon (x20)</div>
        <CButton
          label="Push Notification Balloon (x20)"
          onClick={handlePushBulk}
        />

        <div>Toggle Controlled Notification Balloon</div>
        <CButton
          label="Toggle Controlled Notification Balloon"
