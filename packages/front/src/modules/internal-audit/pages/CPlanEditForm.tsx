...
export function CPlansEditForm() {
  ...
  const fetcher = useCallback(async () => {
    const res = await Api.GET("/u/internal-audit/plan/item/{id}", {
      params: { path: { id } },
    });
    return res;
  }, [id]);
  ...
  const handleSubmit = useCallback(
    async (body: IDtoPlanRequest) => {
      const res = await Api.PUT("/u/internal-audit/plan/item/{id}", {
        params: { path: { id } },
        body,
      });
      apiToast(res);
  ...
  return (
    <CBody breadcrumbs={breadcrumbs}>
      <CAsyncLoader data={data}>
        {(payload) => (
          <CPlansForm initialData={payload} onSubmit={handleSubmit} />
        )}
      </CAsyncLoader>
    </CBody>
  );
