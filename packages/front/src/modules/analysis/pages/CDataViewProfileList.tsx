...
export function CDataViewProfileList() {
  ...
  const fetcher = useCallback(async () => {
    return await Api.GET("/u/analysis/data-view/profile");
  }, []);
  ...
  const handleDelete = useCallback(
    async (record: IDtoDataViewProfileResponse) => {
      await push(
        ...
        async () => {
          const res = await Api.DELETE("/u/analysis/data-view/profile/{id}", {
    ...
    (d) => [
      {
        icon: ChartLine,
        label: t("values"),
        path: `/analyses/data-view/values/${d.id}`,
      },
      {
        icon: Pencil,
        label: t("edit"),
        path: `/analyses/data-view/profile/item/${d.id}`,
      },
      {
        icon: Trash2,
        label: t("_delete"),
  ...
  return (
    <CBody breadcrumbs={breadcrumbs}>
      <CLine className="justify-end space-x-2 grow">
        <CLinkAdd path="/analyses/data-view/profile/item-add" />
